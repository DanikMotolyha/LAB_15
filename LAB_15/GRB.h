...
namespace GRB
{
	struct Rule		// правило а грамматике Грейбах
	{
		GRBALPHABET nn;		// нетерминал (левый символ правила) < 0
		int iderror;		// идентификатор дигностического сообщения
		...
		char* getCRule(			// получить правила в виде N->цепочка (для распечатки)
			char* b,			// буфер
			short nchain		// номер цепочки (правой части) в правиле
		);
		//short Rule::getNextChain
		short getNextChain(	// получисть следующую за j подходящую цепочку вернуть ее номер или номер -1
