...
namespace LT
{
	...
	void table(In::IN in, LT::LexTable *lt, IT::IdTable *it)
	{
		...
		int k = 1;
		bool flag_separate = false, FLAG = false, lex_recognized = false, flag_yes_main = false;
		int i = 0; string buf;
		if (in.text[0] == ' ') i++;
		while (in.text[i] != '\0')
		{
					...
					for (int i = 0; i <= 16; i++)
					{
						...
						if (FST::execute(a[i]))
						{
							...
							case 5:  {
								...
								if (flag_yes_main) throw ERROR_THROW_IN(103, k, -1);
								flag_yes_main = true;
							} break;
							case 6:  { if (lt->size > 1)
							{
...
#pragma endregion
		...
		if (!flag_yes_main) throw ERROR_THROW(102);
	}
