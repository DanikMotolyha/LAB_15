...
namespace MFST
{
	...
	struct Mfst					// магазинный автомат
	{
		...
		bool reststate();											// ввостановить состояние автомата
		...
		if (lenta_position < lenta_size)
		{
			if (ISNS(st.top()))					// если терминал то false 
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)		// присваивается правило по st.top
				{
					...
					{
						MFST_TRACE1
							savestate(); st.pop();	// сохраняет состояние автомата + удаляет верхний элемент в стеке
					...
					else
					{
						MFST_TRACE4("TNS_NORULECHAIN/NS_NORULE")
							savediagnosis(NS_NORULLECHAIN); rc = reststate() ? NS_NORULLECHAIN : NS_NORULE;
					};
				}
				else rc = NS_ERROR;
			}
			...
			else { MFST_TRACE4("TS_NOK/NS_NORULECHAIN") rc = reststate() ? TS_NOK : NS_NORULLECHAIN; };
		}
	...
	bool Mfst::start()
	{
		...
		rc_step = step();
		while (rc_step == NS_OK || rc_step == NS_NORULLECHAIN || rc_step == TS_OK || rc_step == TS_NOK) rc_step = step();
